Shader "Rimlight_Transparent_Terra"
{
	Properties 
	{ 
		...
		_Alpha ("Alpha", Range (8.0, 0.01)) = 1.0
		_AlphaOffset ("Alpha Offset", Range (4.0, 0.01)) = 0.1
		_EmissionOffset ("Emission Offset", Range (0.5, 0.00)) = 0.1
	}
	...
	SubShader 
	{
		Pass 
		{
			...
			fixed4 frag( v2f i ) : COLOR
			{
				...
				color.xyz = _SpecColor * pow( spec, _SpecPower ) * _Shininess;
				color.xyz = ( _RimColor.rgb * _EmissionOffset ) * rimpower * inAlpha * _RimColor.a;
				color.a = ( rimpower * _AlphaOffset ) * inAlpha;
